...
public class DashScopeLlmService extends AbstractLlmService {
    ...
    public Model newChatModel(LlmChatRequest request) {
        // Build GenerateOptions with streaming enabled
        GenerateOptions.Builder optionsBuilder = GenerateOptions.builder();

        // Add additional parameters if present
        if (request.getHeaders() != null) {
            optionsBuilder.additionalHeaders(request.getHeaders());
        }
        if (request.getQueryParams() != null) {
            optionsBuilder.additionalQueryParams(request.getQueryParams());
        }
        if (request.getBodyParams() != null) {
            optionsBuilder.additionalBodyParams(request.getBodyParams());
        }
