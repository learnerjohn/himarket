#!/usr/bin/env bash
# Higress MCP 统一配置初始化钩子
# 由 deploy.sh 在部署就绪后自动调用
# 继承环境变量: NS
# 
# 功能：根据 higress-mcp.json 配置，批量初始化所有 MCP 服务
# 支持类型：
#   - OPEN_API: 基于 OpenAPI YAML 配置的 MCP 服务
#   - DIRECT_ROUTE: 直接路由方式的 MCP 服务
...
########################################
# 获取 Higress Console Service IP
########################################
get_higress_host() {
  log "获取 Higress Console Service LoadBalancer IP..."
  
  HIGRESS_HOST=$(kubectl get svc higress-console -n "${NS}" -o jsonpath='{.status.loadBalancer.ingress[0].ip}' 2>/dev/null || echo "")
  
  if [[ -z "$HIGRESS_HOST" ]]; then
    err "无法获取 Higress Console Service LoadBalancer IP"
    err "请确保集群支持 LoadBalancer 服务类型"
    exit 1
  fi
  
  log "Higress Console 地址: ${HIGRESS_HOST}:8080"
}
...
login_higress() {
  ...
  local login_url="http://${HIGRESS_HOST}:8080/session/login"
...
call_higress_api() {
  ...
  local url="http://${HIGRESS_HOST}:8080${path}"
  local max_attempts=3
  local attempt=1
...
main() {
  ...
  log "========================================"
  log "Higress Console 地址: http://${HIGRESS_HOST}:8080"
  log "========================================"
}
